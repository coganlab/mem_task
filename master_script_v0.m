...
%Specify directories
cd('C:\Users\Richy\Desktop\Memieeg')
cf = pwd;
...
%Chek if data for that block exist
...
csv(row_iterator).fc_onset =  encoding_Onset;
csv(row_iterator).fc_end =  encoding_End;
csv(row_iterator).fc_response =  choice;
csv(row_iterator).fc_rt =  response_time;
...
csv(row_iterator).recall_onset=  elaborate_Onset;
csv(row_iterator).recall_end =  elaborate_End;
csv(row_iterator).recall_response =  choice;
csv(row_iterator).recall_rt =  response_time;
